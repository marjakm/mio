...
use sys::windows::ll::{LinkedList, Entry};
...
struct SelectorInner {
    ...
    level_triggered: Mutex<LinkedList<IoEvent>>,
}
...
impl Selector {
        ...
        CompletionPort::new(1).map(|cp| {
            Selector {
                inner: Arc::new(SelectorInner {
                    ...
                    level_triggered: Mutex::new(LinkedList::new()),
                }),
            }
        })
    }
    ...
    pub fn select(&mut self,
        ...
        for event in inner.level_triggered.lock().unwrap().iter() {
            dst.push(*event);
        }
...
impl Registration {
    ...
    pub fn push_event(&mut self, set: EventSet, events: &mut Vec<IoEvent>) {
        ...
        if set != EventSet::none() {
            ...
            if self.opts.is_edge() {
                ...
                match self.level {
                    Some(ref entry) => {
                        let e = level.get_mut(entry);
                        e.kind = e.kind | event.kind;
                    }
                    None => {
                        self.level = Some(level.push(event));
                    }
                }
            }
        }
    ...
    pub fn unset_readiness(&mut self, set: EventSet) {
        ...
        if let Some(entry) = self.level.take() {
            let selector = self.selector.as_ref()
                .expect("expected selector");

            let mut ll = selector.level_triggered.lock().unwrap();
            let remove = {
                let event = ll.get_mut(&entry);
                event.kind = event.kind & !set;

                event.kind == EventSet::none()
            };

            if remove {
                ll.remove(entry);
            } else {
                self.level = Some(entry);
            }
        }
    }
    ...
    pub fn deregister(&mut self) {
        ...
        if let Some(entry) = self.level.take() {
            ...
            selector.level_triggered.lock().unwrap().remove(entry);
        }
